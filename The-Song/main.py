...
def get_suggestions():
    data = pd.read_csv('song-data-preprocessed1.csv')
    return list(data['Song-Name'].str.capitalize())
...
def details():
    ...
    title=title.lower()
    data = pd.read_csv('song-data-preprocessed1.csv')
    #print(type(data.loc[data["Song-Name"]==title,['Song-Name','Singer/Artists','Genre','Album/Movie','User-Rating','comb']]))
    details=data.loc[data["Song-Name"]==title,['Song-Name','Singer/Artists','Genre','Album/Movie','User-Rating','comb']]
    #details = details.loc[:,['Song-Name','Singer/Artists','Genre','Album/Movie','User-Rating','comb']]
    ...
    details["SongName"]=details["Song-Name"]
    details["SingerArtists"]=details["Singer/Artists"]
    details["AlbumMovie"]=details["Album/Movie"]
    details["UserRating"]=details["User-Rating"]
    print(details)
...
def recommend():
    ...
    rec_songs = request.form['rec_songs']
    ...
    rec_songs = convert_to_list(rec_songs)
    ...
    # song_cards = {rec_posters[i]: rec_songs[i] for i in range(len(rec_posters))}
    ...
    for i in range(len(rec_posters)):
        #print(rec_songs[i])
        #print(rec_posters[i])
        song_cards[rec_posters[i]]=rec_songs[i]
    print(song_cards)
    ...
    return render_template('recommend.html',title=title,poster=poster,overview="",vote_average="",
        ...
        song_cards=song_cards,reviews={},casts={},cast_details={}, album=album, singers=singers, songname=songname, userrating=userrating)
